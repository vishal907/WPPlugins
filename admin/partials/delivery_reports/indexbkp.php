                ...
                <table id="delivery_report" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        <?php 
								...
								foreach($shipping_methods as $key => $shipments) {
                                $len = count($shipments);
                            ?>
                            <?php for($i=0; $i < $len; $i++) { ?>
                                <tr>
                                   <td><?= $key ?></td>
                                    <td><a href="/wp-admin/post.php?post=<?= $shipments[$i]['id'] ?>&action=edit" target="_blank">#<?= $shipments[$i]['id'] ?></a></td>
									 
                                    <td><?= $shipments[$i]['shipping_address'] ?></td>
                                    <td><?= $shipments[$i]['phone'] ?></td>
                                    <td><?= $shipments[$i]['order_comments'] ?></td>
                                    <td></td>
